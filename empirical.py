...
elif (forecast_from > fred_data_last) & (forecast_from <= tw_data_last):  # (記得要括號，不然會出錯)輸入的欲預測時間forecast_from為9月，超出fred_data_last8月，則不管怎樣fred_data之in-sample都是fred_data_last8月
...
result = forecast2(r_ex, 'level', X, datetime(2021,1,1), datetime(2022,1,1), 12)
plt.plot(result[3])
plt.show()
